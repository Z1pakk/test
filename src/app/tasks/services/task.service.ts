...
export class TaskService {
	...
	constructor(private http: HttpClient) {
	  ...
	  public update(task: Task) {
		return this.http.put<Task>(`https://angular-project-62344.firebaseio.com/users/${task.id}.json`, task).subscribe(() => {
			this.reload();
		});
	  }
	  public userForEdit() {
	  ...
	  public getById(id: string): Observable<Task> {
		return this.http.get<Task>(`https://angular-project-62344.firebaseio.com/taskss/${id}.json`)
			.pipe(map((task: Task) => {
			return {
				...task, id,
			};
